...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .email-card.email-1 { border-color: rgba(25,195,125,0.2); background: rgba(25,195,125,0.04); }
  .email-card.email-2 { border-color: rgba(245,166,35,0.2); background: rgba(245,166,35,0.04); }
  .email-card.email-3 { border-color: rgba(99,102,241,0.2); background: rgba(99,102,241,0.04); }
  .email-card-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
  .email-card-label { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.09em; }
  .email-1 .email-card-label { color: #19C37D; }
  .email-2 .email-card-label { color: #f5a623; }
  .email-3 .email-card-label { color: #a5b4fc; }
  .email-subject { font-size: 12px; font-weight: 600; color: #D6E1EA; margin-bottom: 10px; padding: 7px 10px; background: rgba(255,255,255,0.05); border-radius: 6px; }
  .email-subject-label { font-size: 9px; color: #4a6a82; text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 3px; }
  .email-body { font-size: 12.5px; color: #9FB3C8; line-height: 1.75; white-space: pre-wrap; }
  .copy-btn { font-size: 10px; padding: 4px 10px; border-radius: 5px; border: 1px solid rgba(255,255,255,0.1); background: transparent; color: #4a6a82; cursor: pointer; font-family: 'Inter', sans-serif; transition: all 0.15s; }
  .copy-btn:hover { color: #D6E1EA; border-color: rgba(255,255,255,0.2); }
...
function exportCSV(results) {
  const headers = ['company_name','email','industry','ownership','owner_profiles','owner_hobbies','owner_family','pain_points','tech_stack','recent_news','reviews_negative','reviews_positive','reviews_trend','online_pricing','market_population','market_competition','company_struggles','marketing_current','marketing_agencies','email_1_subject','email_1','email_2_subject','email_2','email_3_subject','email_3','ghl_status']
  const rows = results.filter(r => r.data).map(r => {
    const d = r.data
    return [r.name, r.email, d.industry||'', d.ownership||'', d.owner_profiles||'', d.owner_hobbies||'', d.owner_family||'', d.pain_points||'', d.tech_stack||'', d.recent_news||'', d.reviews_negative||'', d.reviews_positive||'', d.reviews_trend||'', d.online_pricing||'', d.market_population||'', d.market_competition||'', d.company_struggles||'', d.marketing_current||'', d.marketing_agencies||'', d.email_1_subject||'', d.email_1||'', d.email_2_subject||'', d.email_2||'', d.email_3_subject||'', d.email_3||'', r.ghlStatus||'']
...
async function ghlCreateOpportunity(company, contactId, apiKey, locationId, pipelineId, stageId) {
  ...
  if (!res.ok) {
    console.error('Opportunity error:', res.status, text)
    throw new Error(Array.isArray(data.message) ? data.message.join(', ') : data.message || `Opportunity failed (${res.status})`)
  }
...
async function ghlCreateNote(contactId, company, apiKey) {
  const res = await fetch(`/api/ghl/contacts/${contactId}/notes/`, {
    ...
    body: JSON.stringify({ body: buildNoteBody(company) })
  })
  const data = await res.json()
  if (!res.ok) throw new Error(Array.isArray(data.message) ? data.message.join(', ') : data.message || 'Note creation failed')
  return data
}
    ...
    <div className={`company-card ${expanded ? 'active' : ''} ${selected ? 'selected' : ''}`}>
      ...
      {expanded && c.data && (
        <div className="company-card-body">
          ...
          <div className="research-grid" style={{marginTop: 10}}>
            <div className="research-block" style={{gridColumn:'1/-1',borderColor:'rgba(99,102,241,0.15)',background:'rgba(99,102,241,0.04)'}}>
              <div className="research-label" style={{color:'#a5b4fc'}}>📣 Current marketing activity</div>
              <div className="research-value">{c.data.marketing_current || 'Not found'}</div>
            </div>
            <div className="research-block" style={{gridColumn:'1/-1'}}>
              <div className="research-label">🏢 Marketing agencies / vendors</div>
              <div className="research-value">{c.data.marketing_agencies || 'None found'}</div>
            </div>
          </div>

          {/* Email 1 — Pre-call confirmation */}
          {c.data.email_1 && (
            <div className="email-card email-1">
              <div className="email-card-header">
                <div className="email-card-label">✉️ Email 1 — Pre-call confirmation</div>
                <button className="copy-btn" onClick={() => navigator.clipboard.writeText(`Subject: ${c.data.email_1_subject}\n\n${c.data.email_1}`)}>Copy</button>
              </div>
              <div className="email-subject-label">Subject line</div>
              <div className="email-subject">{c.data.email_1_subject}</div>
              <div className="email-body">{c.data.email_1}</div>
            </div>
          )}

          {/* Email 2 — Not a fit */}
          {c.data.email_2 && (
            <div className="email-card email-2">
              <div className="email-card-header">
                <div className="email-card-label">✉️ Email 2 — Not a fit (under $3M)</div>
                <button className="copy-btn" onClick={() => navigator.clipboard.writeText(`Subject: ${c.data.email_2_subject}\n\n${c.data.email_2}`)}>Copy</button>
              </div>
              <div className="email-subject-label">Subject line</div>
              <div className="email-subject">{c.data.email_2_subject}</div>
              <div className="email-body">{c.data.email_2}</div>
            </div>
          )}

          {/* Email 3 — 24hr reminder */}
          {c.data.email_3 && (
            <div className="email-card email-3">
              <div className="email-card-header">
                <div className="email-card-label">✉️ Email 3 — 24hr call reminder</div>
                <button className="copy-btn" onClick={() => navigator.clipboard.writeText(`Subject: ${c.data.email_3_subject}\n\n${c.data.email_3}`)}>Copy</button>
              </div>
              <div className="email-subject-label">Subject line</div>
              <div className="email-subject">{c.data.email_3_subject}</div>
              <div className="email-body">{c.data.email_3}</div>
            </div>
          )}
